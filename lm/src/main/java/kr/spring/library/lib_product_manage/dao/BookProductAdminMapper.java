package kr.spring.library.lib_product_manage.dao;
...
public interface BookProductAdminMapper {
	...
	// @Update("UPDATE lib_product_manage SET
	// lib_product_loanCnt=lib_product_loanCnt+1 WHERE callNumber=#{callNumber}")
	...
	// 대출중인 도서 리스트
	@Select("SELECT * FROM lib_product_manage WHERE lib_product_product_status = #{lib_product_product_status}")
