$(function(){
	...
	function selectList(pageNum){
		...
		$.ajax({
			url:'listReply.do',
			type:'post',
			data:{pageNum:pageNum,rowCount:rowCount,event_board_num:$('#event_board_num').val()},
			dataType:'json',
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					if(item.reply_modify_date){
						output += '<span class="modify-date">최근 수정일 : ' + item.reply_modify_date + '</span>'; 
					}else{
	...
	$(document).on('click','.modify-btn',function(){
		...
		modifyUI += '<textarea row="3" cols="50" name="reply_content" id="mre_content" class="rep-content">'+content+'</textarea>';
	...
	//초기 데이터(목록) 호출
	let category_num = $('#event_board_category').val();
	if(category_num == 1){
		selectList(1);	
	}
